/*
  ...
  hacked up by Orick22. Expect it to work for wifi credentials. will sometimes fail out around TOS. but if it gets past TOS, will work to enrollment and Update. 
  fail at TOS might be due to loading issue after wifi credentials are entered. source of bug was never discovered for the 70+ V86 chromebooks I had to enroll 
  since it happened too infrequently to troubleshoot.
...
void enterEnrollment() {
  wait(60); //checking for updates.  60s is long, so adjust as needed
  Keyboard.write(KEY_TAB);
  wait(1);
  Keyboard.write(KEY_TAB);
  wait(1);
  ...
  wait(1);
  Keyboard.write(KEY_ENTER);
  wait(60);   // most CBs don't take this long.  adjust as needed
  Keyboard.write(KEY_ENTER);
}
